...
import { formatSubject } from "@core/utils/queueHelpers";
...
function getAuditNodeColor(action: string) {
  switch (action) {
    case "JOB_INGESTED":
      return "cyan";
    case "CREATED":
...
function getAuditNodeIcon(action: string) {
  switch (action) {
    case "COMPLETED":
    case "CREATED":
    case "WALKIN_CREATED":
...
export default function AdminQueuePanel() {
  ...
  useEffect(() => {
    ...
    const handler = (e: MouseEvent) => {
      if (hamburgerDropdownRef.current && !hamburgerDropdownRef.current.contains(e.target as Node)) {
    ...
    useQuery<AdminJobsResponse>({
      ...
      queryFn: () => {
        ...
        return queueApi.getAdminJobs({
          ...
          assignedTo: assignedToFilter.length > 0 ? assignedToFilter.join(',') : undefined,
          date: effectiveDate,
        });
      },
  ...
  const handleDelete = (jobId: string, callback?: () => void) => {
    if (
      window.confirm(
  ...
  return (
    <div className="admin-queue-page">
      ...
      {isMobile && isSelectionMode && selectedJobs.size > 0 && (
        <div 
          className="mobile-sticky-selection-bar" 
          style={{
            position: 'fixed',
            bottom: '20px',
            left: '5%',
        ...
        >
          <div style={{ display: 'flex', flexDirection: 'column' }}>
            <span style={{ color: '#94a3b8', fontSize: '0.65rem', fontWeight: 800, textTransform: 'uppercase', letterSpacing: '0.05em' }}>Selected</span>
            <span style={{ color: 'white', fontSize: '0.95rem', fontWeight: 800 }}>{selectedJobs.size} {selectedJobs.size === 1 ? 'Job' : 'Jobs'}</span>
          </div>
          ...
          <div style={{ display: 'flex', gap: '0.4rem' }}>
            {activeTab !== "JUNK" ? (
              <button
                ...
                style={{ background: '#ef4444', color: 'white', border: 'none', borderRadius: '0.5rem', padding: '0.5rem 0.75rem', fontSize: '0.75rem', fontWeight: 800, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '0.25rem' }}
              >
                🗑 JUNK
              </button>
            ) : (
              <button
                ...
                style={{ background: '#10b981', color: 'white', border: 'none', borderRadius: '0.5rem', padding: '0.5rem 0.75rem', fontSize: '0.75rem', fontWeight: 800, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '0.25rem' }}
              >
                ↩ RESTORE
              </button>
            )}
            ...
            {activeTab !== "ADMIN_REVIEW" && (
              <button
                ...
                style={{ background: '#f59e0b', color: 'white', border: 'none', borderRadius: '0.5rem', padding: '0.5rem 0.75rem', fontSize: '0.75rem', fontWeight: 800, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '0.25rem' }}
              >
                ⚠ REVIEW
              </button>
            )}
            ...
            <button
              ...
              style={{ background: '#dc2626', color: 'white', border: 'none', borderRadius: '0.5rem', padding: '0.5rem 0.75rem', fontSize: '0.75rem', fontWeight: 800, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '0.25rem' }}
            >
              🔥 DELETE
            </button>
          </div>
      ...
      <header className="admin-queue-header">
        <div className="header-left-hub">
          ...
          <div className="header-titles">
            <h1 className="admin-queue-title">{isMobile ? `Queue: ${TAB_LABELS[activeTab] || "Waiting"}` : "QUEUE CONTROL CENTER"}</h1>
          </div>
        </div>
        ...
        {isMobile && (
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', position: 'relative' }}>
            <button 
              ...
              style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#1e293b', padding: '0.5rem', display: 'flex', alignItems: 'center', justifyContent: 'center' }}
            >
            ...
            {hamburgerOpen && (
              <div 
                className="mobile-hamburger-dropdown" 
                ...
                style={{ 
                  ...
                  padding: '0.75rem', display: 'flex', flexDirection: 'column', gap: '0.35rem', 
                  zIndex: 1000, width: '220px' 
                }}
              >
                <span style={{ fontSize: '0.68rem', fontWeight: 800, color: '#94a3b8', padding: '0 0.5rem', textTransform: 'uppercase', letterSpacing: '0.05em' }}>Switch Section</span>
                {[
                  { id: 'QUEUED', label: 'Waiting', count: stats?.totalQueued, color: '#3b82f6' },
                  { id: 'ASSIGNED', label: 'In Progress', count: stats?.totalInProgress, color: '#10b981' },
                  { id: 'COMPLETED', label: 'Finished', count: stats?.completed, color: '#64748b' },
                  { id: 'ADMIN_REVIEW', label: 'Review', count: stats?.adminReview, color: '#ef4444' },
                  { id: 'JUNK', label: 'Junk', count: stats?.junk, color: '#6b7280' },
                  { id: 'LOAD', label: 'Staff Load', count: stats?.activeSessions, color: '#8b5cf6' },
                ].map(tab => (
                  <button
                    key={tab.id}
                    onClick={() => {
                  ...
                  >
                    <span>{tab.label}</span>
                    {tab.count > 0 && (
                      <span style={{ background: tab.color, color: 'white', fontSize: '0.62rem', padding: '1px 5px', borderRadius: '10px', fontWeight: 800 }}>
                        {tab.count}
                      </span>
                    )}
                  </button>
                ...
                <span style={{ fontSize: '0.68rem', fontWeight: 800, color: '#94a3b8', padding: '0 0.5rem', textTransform: 'uppercase', letterSpacing: '0.05em' }}>Actions</span>
                <Link 
                  to="/admin/whatsapp-job" 
                  onClick={() => setHamburgerOpen(false)}
                  style={{ display: 'flex', gap: '0.5rem', alignItems: 'center', padding: '0.55rem 0.75rem', borderRadius: '0.5rem', color: '#475569', fontWeight: 600, fontSize: '0.78rem', textDecoration: 'none' }}
                >
                  📱 WhatsApp Entry
                </Link>
                <button 
                  onClick={() => { setShowMessages(true); setHasUnread(false); setHamburgerOpen(false); }}
                  style={{ display: 'flex', gap: '0.5rem', alignItems: 'center', padding: '0.55rem 0.75rem', borderRadius: '0.5rem', border: 'none', background: 'transparent', color: '#475569', fontWeight: 600, fontSize: '0.78rem', cursor: 'pointer', width: '100%', textAlign: 'left' }}
                >
                  💬 Messages {hasUnread && '🔴'}
                </button>
                <button 
                  onClick={() => { setShowSettingsModal(true); setHamburgerOpen(false); }}
                  style={{ display: 'flex', gap: '0.5rem', alignItems: 'center', padding: '0.55rem 0.75rem', borderRadius: '0.5rem', border: 'none', background: 'transparent', color: '#475569', fontWeight: 600, fontSize: '0.78rem', cursor: 'pointer', width: '100%', textAlign: 'left' }}
                >
                  ⚙ Settings
                </button>
      ...
      <div className="admin-main-grid">
        <div className="admin-queue-section">
                  ...
                  <>
                    ...
                    {!isSelectionMode && (
                      <div className="aqt-filters-static-row">
                        ...
                        <div className="filter-select-hub" style={{ position: 'relative' }} ref={staffDropdownRef}>
                          <button
                            className="select-elite"
                            style={{ display: 'flex', alignItems: 'center', gap: '0.4rem', cursor: 'pointer', minWidth: '160px', justifyContent: 'space-between' }}
                            ...
                            <span>
                              {assignedToFilter.length === 0
                                ...
                                : assignedToFilter.length === 1
                                  ? (staffList?.find((s: any) => s._id === assignedToFilter[0])?.name || '1 selected')
                                  : `${assignedToFilter.length} selected`}
                            </span>
                            <svg width="12" height="12" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                          ...
                          {staffDropdownOpen && (
                            ...
                            >
                              <div
                                ...
                                onClick={() => { setAssignedToFilter([]); setStaffDropdownOpen(false); setPage(1); }}
                              >
                                ✕ Clear selection
                              </div>
                              {staffList?.map((s: any) => {
                                const checked = assignedToFilter.includes(s._id);
                                return (
                                  <label
                                    key={s._id}
          ...
          {/* Mobile Filters Area */}
          {isMobile && activeTab !== "LOAD" && (
            <div className="mobile-queue-filters-row" style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem', width: '100%', padding: '0.75rem', background: '#f8fafc', borderBottom: '1px solid #e2e8f0' }}>
              <div style={{ display: 'flex', gap: '0.5rem', width: '100%' }}>
                <div style={{ flex: 1, background: 'white', padding: '0 0.75rem', borderRadius: '0.75rem', border: '1px solid #cbd5e1', display: 'flex', alignItems: 'center', gap: '0.35rem', height: '40px' }}>
                  ...
                  />
                  {searchTerm && <button onClick={() => { setSearchTerm(''); setPage(1); }} style={{ background: 'none', border: 'none', cursor: 'pointer', fontSize: '0.75rem', color: '#94a3b8', fontWeight: 800 }}>✕</button>}
                </div>
                <button
                  className={`btn-selection-toggle ${isSelectionMode ? "active" : ""}`}
                  onClick={() => {
                  ...
                  style={{
                    ...
                    border: `1px solid ${isSelectionMode ? 'var(--q-primary)' : '#cbd5e1'}`,
                    background: isSelectionMode ? '#eff6ff' : 'white',
                    color: isSelectionMode ? 'var(--q-primary)' : '#64748b',
                    fontWeight: 700,
                    fontSize: '0.75rem',
                    display: 'flex',
                    alignItems: 'center',
              ...
              <div style={{ display: 'flex', gap: '0.5rem', width: '100%' }}>
                <div style={{ flex: 1, position: 'relative' }} ref={staffDropdownRef}>
                  <button
                    className="select-elite"
                    onClick={() => setStaffDropdownOpen(o => !o)}
                    style={{ width: '100%', height: '40px', padding: '0 0.75rem', borderRadius: '0.75rem', border: '1px solid #cbd5e1', background: 'white', display: 'flex', alignItems: 'center', justifyContent: 'space-between', fontSize: '0.78rem', fontWeight: 700, cursor: 'pointer' }}
                  >
                    <span>
                      {assignedToFilter.length === 0
                        ? 'All Designers'
                  ...
                  {staffDropdownOpen && (
                    <div
                      style={{
                        position: 'absolute', top: '110%', left: 0, zIndex: 999,
                        background: '#fff', border: '1px solid #e2e8f0', borderRadius: '0.5rem',
                        boxShadow: '0 4px 16px rgba(0,0,0,0.12)', width: '100%', minWidth: '200px', padding: '0.5rem 0'
                      }}
                    >
                      <div
                        style={{ padding: '0.4rem 0.75rem', cursor: 'pointer', fontWeight: 600, fontSize: '0.8rem', color: '#64748b', borderBottom: '1px solid #f1f5f9' }}
                      ...
                      {staffList?.map((s: any) => {
                        ...
                        return (
                          <label
                            key={s._id}
                            style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', padding: '0.4rem 0.75rem', cursor: 'pointer', fontSize: '0.82rem', background: checked ? '#f0f7ff' : 'transparent', color: '#1e293b' }}
                          >
                            <input
                              type="checkbox"
                              checked={checked}
                ...
                {activeTab === "COMPLETED" && (
                  <div style={{ flex: 1 }}>
                    <input
                      ...
                      style={{ width: '100%', height: '40px', padding: '0 0.75rem', borderRadius: '0.75rem', border: '1px solid #cbd5e1', background: 'white', fontWeight: 700, fontSize: '0.78rem' }}
                    />
                  </div>
                )}
              </div>
          ...
          {activeTab === "LOAD" ? (
            ...
            <div className="aqt-table-wrapper">
                                ...
                                <button
                                  ...
                                  onClick={() => {
                                    setChatSettings({
                                      ...
                                      jobId: job._id,
                                      prefill: `Re Job #${String(job._id).slice(-6).toUpperCase()}: `,
                                    });
              ...
              /* Mobile Cards List */
              activeTab === "QUEUED" ? (
                <div className="mobile-queue-job-cards-list animate-fade-in" style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem', padding: '0.75rem' }}>
                  {!queueData?.jobs || queueData.jobs.length === 0 ? (
                    <div style={{ textAlign: 'center', padding: '3rem', color: '#94a3b8', fontSize: '0.85rem', fontWeight: 600 }}>No jobs in this queue.</div>
                  ) : (
                    queueData.jobs.map((job: any) => {
                      const { clean: subjectClean } = formatSubject(job.emailSubject || "");
                      return (
                        <div
                          key={job._id}
                          className={`mobile-queue-job-card priority-${job.priorityScore >= 20 ? 'imm' : job.priorityScore >= 10 ? 'crit' : job.priorityScore >= 5 ? 'urg' : 'norm'} ${selectedJobs.has(job._id) ? 'selected' : ''}`}
                          onClick={() => {
                            if (isSelectionMode) toggleSelection(job._id);
                            else setShowViewModal(job);
                          }}
                          style={{
                            background: selectedJobs.has(job._id) ? '#eff6ff' : 'white',
                            border: `1px solid ${selectedJobs.has(job._id) ? 'var(--q-primary)' : '#e2e8f0'}`,
                            borderRadius: '0.75rem',
                            padding: '0.75rem',
                            display: 'flex',
                        ...
                        >
                          ...
                          {/* Row 1: Customer & Flags + Timing */}
                          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', paddingLeft: isSelectionMode ? '1.75rem' : '0' }}>
                            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.2rem', flex: 1 }}>
                              <div style={{ display: 'flex', alignItems: 'center', gap: '0.35rem', flexWrap: 'wrap' }}>
                                <span style={{ fontWeight: 800, color: '#0f172a', fontSize: '0.85rem' }}>{job.customerName}</span>
                                {job.type === "WHATSAPP" && <span className="aqt-source-badge wa">📱 WA</span>}
                                {(job.type === "WALKIN" || job.emailSubject?.includes("Walk-in")) && <span className="aqt-source-badge walkin">🚶 WALK-IN</span>}
                                {job.threadId && <span className="aqt-flag-badge revision">↺ REV</span>}
                                {job.lastPausedBy && <span className="aqt-flag-badge prior">PRIOR</span>}
                                {job.pinnedToStaff && <span className="aqt-flag-badge pinned">📌 {job.pinnedToStaff.name.split(" ")[0]}</span>}
                              </div>
                              {(subjectClean || job.emailSubject) && (
                            ...
                            {/* Timing */}
                            <div style={{ fontSize: '0.7rem', color: '#475569', textAlign: 'right', fontWeight: 700 }}>
                          ...
                          {/* Row 2: Select Assignment + Select Priority side by side */}
                          <div style={{ display: 'flex', gap: '0.5rem', marginTop: '0.25rem' }} onClick={e => e.stopPropagation()}>
                              ...
                              <select
                                ...
                                onChange={(e) => {
                                  ...
                                  if (staffId === "none") {
                                    ...
                                    if (
                                      !onlineStaffIds.has(staffId) &&
                                      !window.confirm("Designer is OFFLINE. Pin anyway?")
                                    )
                                      return;
                                    pinJobMutation.mutate({ jobId: job._id, staffId });
                                  }
                                }}
                                style={{ width: '100%', height: '36px', borderRadius: '0.5rem', border: '1px solid #cbd5e1', background: 'white', fontWeight: 700, fontSize: '0.75rem', padding: '0 0.5rem' }}
                              >
                                <option value="none">— Pool —</option>
                                {assignmentStaffList.map((s: any) => {
                                  const sess = (Array.isArray(sessions) ? sessions : []).find((se: any) => (se.staffId?._id || se.staffId) === s._id);
                                  const loadInfo = sess ? ` (${sess.pinnedJobs?.length || 0})` : "";
                                  return (
                                    <option key={s._id} value={s._id}>
                                      {onlineStaffIds.has(s._id) ? "🟢" : "⚪"} {s.name}{loadInfo}
                                    </option>
                                  );
                                })}
                              </select>
                            ...
                            <div style={{ flex: 1 }}>
                              <select
                                ...
                                style={{ width: '100%', height: '36px', borderRadius: '0.5rem', fontWeight: 750, fontSize: '0.75rem', padding: '0 0.5rem' }}
                              >
                                <option value="0">NORMAL</option>
                                <option value="5">URGENT</option>
                                <option value="10">CRITICAL</option>
                ...
                /* Fallback Mobile Cards List for other tabs */
                <div className="mobile-queue-other-cards-list animate-fade-in" style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem', padding: '0.75rem' }}>
                  {!queueData?.jobs || queueData.jobs.length === 0 ? (
                    <div style={{ textAlign: 'center', padding: '3rem', color: '#94a3b8', fontSize: '0.85rem', fontWeight: 600 }}>No jobs in this queue.</div>
                  ) : (
                    queueData.jobs.map((job: any) => {
                      ...
                      return (
                        <div
                          key={job._id}
                          className={`mobile-queue-other-card ${selectedJobs.has(job._id) ? 'selected' : ''}`}
                          onClick={() => {
                            if (isSelectionMode) toggleSelection(job._id);
                            else setShowViewModal(job);
                          }}
                        ...
                        >
                          ...
                          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', paddingLeft: isSelectionMode ? '1.75rem' : '0' }}>
                            <span style={{ fontWeight: 800, color: '#0f172a', fontSize: '0.85rem' }}>{job.customerName}</span>
                            <span className={`aqt-status-pill ${job.status}`} style={{ fontSize: '0.62rem', padding: '2px 6px', borderRadius: '4px' }}>
                              {job.status}
                            </span>
                          </div>
                          ...
                          {activeTab === "ASSIGNED" && job.status === "PAUSED" && (
                            <div
                              style={{
                                background: "#fffbeb",
                            ...
                            >
                              <span>⚠️</span>
                              <span>HOLD: {job.pauseReason || "General Hold"}</span>
                            </div>
                          )}

                          {activeTab === "ADMIN_REVIEW" && job.returnReason && (
                            <div
                              style={{
                                background: "#fef2f2",
                            ...
                            >
                              <span>⚠️</span>
                              <span>REASON: {job.returnReason}</span>
                            </div>
                          )}

                          <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.7rem', color: '#475569', marginTop: '0.25rem' }}>
                            <span>Assigned: <strong>{job.assignedTo?.name || 'Pool'}</strong></span>
                            <span>{new Date(job.createdAt).toLocaleDateString([], { month: "short", day: "numeric" })}</span>
                          </div>
                        </div>
                      );
                    })
          ...
          <div className="modal-content-luxury" onClick={(e) => e.stopPropagation()} style={{ maxWidth: "500px" }}>
            ...
            <div className="modal-scroll-area" style={{ padding: "1.5rem" }}>
               <div className="sidebar-stats-grid" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "1rem" }}>
                  ...
                  <div className="stat-box-vertical green" onClick={() => { setActiveTab("QUEUED"); setPage(1); setSelectedJobs(new Set()); setShowStatsModal(false); }}>
                    <span className="stat-box-label">Live Load</span>
                    ...
                    return (
                      <tr key={log._id}>
                        <td>
                          ...
                          <br />
                          <small>{String(log.jobId || log._id).slice(-6)}</small>
                        </td>
                        <td>
                          <span
            ...
            <div className="modal-scroll-area" style={{ padding: "2rem" }}>
              <div className="audit-timeline">
                {showJobAuditModal.auditLog?.map((log: any, i: number) => (
                  <div key={i} className="audit-timeline-entry">
                    <div className={`audit-timeline-node ${getAuditNodeColor(log.action)}`}>
                      {getAuditNodeIcon(log.action)}
                    </div>
      ...
      {showViewModal && (
        <JobDetailModal
          ...
          onDeleteJob={(jobId: string) => {
            handleDelete(jobId, () => setShowViewModal(null));
          }}
        />
      )}
      ...
      {showSettingsModal && (
        <div className="modal-overlay" onClick={() => setShowSettingsModal(false)}>
          <div className="modal" onClick={e => e.stopPropagation()} style={{ maxWidth: '500px', maxHeight: '85vh', display: 'flex', flexDirection: 'column' }}>
            <div className="modal-header-luxury">
              <h2>SYSTEM SETTINGS</h2>
              <button className="close-btn" onClick={() => setShowSettingsModal(false)}>×</button>
            </div>
            
            <div className="modal-body-scrollable" style={{ padding: '1.5rem', overflowY: 'auto', flex: 1 }}>
               <div className="settings-section-premium">
                 ...
                 <div className="settings-item-elite" style={{ marginTop: '1.5rem', borderTop: '1px solid #e2e8f0', paddingTop: '1.5rem' }}>
                   <div className="settings-info">
                     <div className="settings-label">Queue Completion Behavior</div>
                     <div className="settings-desc">Determine action when Queue Staff marks a job complete.</div>
                   </div>
                   <div className="settings-action">
                     <select
                       className="search-input-elite"
                       style={{ width: '220px', padding: '0.5rem', outline: 'none', border: '1.2px solid #cbd5e1', borderRadius: '0.5rem', fontWeight: 600, fontSize: '0.85rem' }}
                       value={configs?.find((c: any) => c.key === 'queueCompletionBehavior')?.value || 'QUEUE_ONLY'}
                       onChange={(e) => updateConfigMutation.mutate({ key: 'queueCompletionBehavior', value: e.target.value })}
                     >
                       <option value="QUEUE_ONLY">Queue Only</option>
                       <option value="PROMPT">Prompt for Job Creation</option>
                       <option value="AUTO">Automatically Open Job Creation</option>
                     </select>
                   </div>
                 </div>
                 ...
                 <div style={{ marginTop: '2rem', borderTop: '1px solid #e2e8f0', paddingTop: '1.5rem' }}>
                    <div className="settings-label" style={{ marginBottom: '1rem' }}>Hold Reasons & Rules</div>
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
                      {configs?.find((c: any) => c.key === 'hold_reasons')?.value?.map((reason: any, ridx: number) => (
                        <div key={reason.id} style={{ background: '#f8fafc', padding: '1rem', borderRadius: '0.75rem', border: '1px solid #e2e8f0' }}>
                          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '0.5rem' }}>
                            <span style={{ fontWeight: 800, fontSize: '0.85rem' }}>{reason.label}</span>
                            <button 
                              style={{ color: '#ef4444', background: 'none', border: 'none', cursor: 'pointer', fontSize: '0.75rem', fontWeight: 700 }}
                              onClick={() => {
                                const current = configs.find((c: any) => c.key === 'hold_reasons').value;
                                const updated = current.filter((_: any, i: number) => i !== ridx);
                                updateConfigMutation.mutate({ key: 'hold_reasons', value: updated });
                              }}
                            >
                              REMOVE
                            </button>
                          ...
                          <div style={{ display: 'flex', gap: '1rem', alignItems: 'center' }}>
                            <label style={{ display: 'flex', alignItems: 'center', gap: '0.4rem', fontSize: '0.7rem', fontWeight: 600, cursor: 'pointer' }}>
                              <input 
                                type="radio" 
                                name={`behavior-${reason.id}`}
                                checked={reason.behavior === 'RETURN_TO_POOL'} 
                                onChange={() => {
                                  const current = configs.find((c: any) => c.key === 'hold_reasons').value;
                                  current[ridx].behavior = 'RETURN_TO_POOL';
                                  if (current[ridx].timeLimit === null || current[ridx].timeLimit === undefined) {
                                    current[ridx].timeLimit = 15;
                                  }
                                  updateConfigMutation.mutate({ key: 'hold_reasons', value: [...current] });
                                }}
                              />
                              RETURN TO POOL
                            </label>
                            <label style={{ display: 'flex', alignItems: 'center', gap: '0.4rem', fontSize: '0.7rem', fontWeight: 600, cursor: 'pointer' }}>
                              <input 
                                type="radio" 
                                name={`behavior-${reason.id}`}
                                checked={reason.behavior === 'STAY_HOLD'} 
                                onChange={() => {
                                  const current = configs.find((c: any) => c.key === 'hold_reasons').value;
                                  current[ridx].behavior = 'STAY_HOLD';
                                  current[ridx].timeLimit = null;
                                  updateConfigMutation.mutate({ key: 'hold_reasons', value: [...current] });
                                }}
                              />
                              STAY HOLD
                            </label>
                            {reason.behavior !== 'STAY_HOLD' && (
                              <div style={{ marginLeft: 'auto', display: 'flex', alignItems: 'center', gap: '0.4rem', fontSize: '0.7rem', fontWeight: 600 }}>
                                <span>LIMIT (MINS):</span>
                                <input 
                                  type="number" 
                                  min="1"
                                  value={reason.timeLimit || ''} 
                                  onChange={(e) => {
                                    const current = configs.find((c: any) => c.key === 'hold_reasons').value;
                                    current[ridx].timeLimit = parseInt(e.target.value) || 0;
                                    updateConfigMutation.mutate({ key: 'hold_reasons', value: [...current] });
                                  }}
                                  style={{ width: '60px', padding: '0.2rem', border: '1px solid #cbd5e1', borderRadius: '0.25rem', textAlign: 'center' }}
                                />
                              </div>
                            )}
                          </div>
                      ...
                      <button 
                        ...
                        style={{ padding: '0.5rem', fontSize: '0.75rem', marginTop: '0.5rem' }}
                        onClick={() => {
                          const label = prompt('Enter hold reason label:');
                          if (!label) return;
                          const behaviorStr = prompt('Enter behavior (1 for RETURN_TO_POOL, 2 for STAY_HOLD):', '1');
                          const behavior = (behaviorStr || '1') === '2' ? 'STAY_HOLD' : 'RETURN_TO_POOL';
                          let timeLimit = null;
                          if (behavior === 'RETURN_TO_POOL') {
                            const timeLimitStr = prompt('Enter time limit in minutes:', '15');
                            timeLimit = parseInt(timeLimitStr || '15') || 15;
                          }
                          const current = configs.find((c: any) => c.key === 'hold_reasons')?.value || [];
                          const updated = [...current, { id: label.toLowerCase().replace(/\s/g, '_'), label, behavior, timeLimit }];
                          updateConfigMutation.mutate({ key: 'hold_reasons', value: updated });
                        }}
                      >
                        + ADD NEW HOLD REASON
  ...
  return (
    <div className="modal-overlay">
      ...
      >
        ...
        >
          <strong>Job:</strong> #{String(job._id).slice(-6).toUpperCase()} -{" "}
          {job.customerName}
          <br />
        ...
        <div className="modal-header-elite">
          <div className="header-info">
            ...
            <span className="modal-ref-elite">
              #REF: {String(job._id).slice(-6).toUpperCase()}
            </span>
          </div>
    ...
    <div className="modal-overlay" onClick={onClose}>
      ...
      >
        ...
        <div className="modal-body-elite">
          ...
          >
            {showAudit ? "Hide Audit Trail & Activity History ▲" : "📋 View Audit Trail & Activity History ▼"}
          </button>

          {showAudit && (
            <div
              className="modal-audit-timeline-container animate-fade-in"
              style={{
                background: "#f8fafc",
                padding: "1rem",
                borderRadius: "0.75rem",
            ...
            >
              {job.auditLog && job.auditLog.length > 0 ? (
                <div className="audit-timeline">
                  {job.auditLog.map((log: any, i: number) => (
                    <div key={i} className="audit-timeline-entry" style={{ display: 'flex', gap: '0.75rem', marginBottom: '1.25rem' }}>
                      <div className={`audit-timeline-node ${getAuditNodeColor(log.action)}`} style={{ flexShrink: 0, width: '28px', height: '28px', display: 'flex', alignItems: 'center', justifyContent: 'center', borderRadius: '50%' }}>
                        {getAuditNodeIcon(log.action)}
                      </div>
                      <div className="audit-timeline-content" style={{ flex: 1 }}>
                        <div className="audit-time" style={{ fontSize: '0.7rem', color: '#64748b', display: 'flex', alignItems: 'center', gap: '0.2rem', marginBottom: '0.15rem' }}>
                          <span>{new Date(log.timestamp).toLocaleString()}</span>
                        </div>
                        <div className="audit-action-title" style={{ fontSize: '0.78rem', fontWeight: 700, color: '#1e293b' }}>
                          {log.action}
                          {log.actor?.name && (
                            <span className="audit-actor-badge" style={{ marginLeft: '0.4rem', background: '#e2e8f0', color: '#475569', fontSize: '0.62rem', padding: '1px 5px', borderRadius: '4px', fontWeight: 800 }}>
                              {log.actor.name}
                            </span>
                          )}
                        </div>
                        {log.details && (
                          <div className="audit-detail-card" style={{ fontSize: '0.75rem', color: '#475569', marginTop: '0.25rem' }}>
                            {renderAuditDetails(log.action, log.details)}
                          </div>
                        )}
                      </div>
                ...
                <div style={{ color: "#94a3b8", fontStyle: "italic", fontSize: "0.8rem", textAlign: "center", padding: "1.5rem" }}>
        ...
        {!jobs || jobs.length === 0 ? (
          ...
          jobs.map(j => (
            <div key={j._id} className="si-job-pill">
              ...
              <div className="si-job-id">#{String(j._id).slice(-6).toUpperCase()}</div>
